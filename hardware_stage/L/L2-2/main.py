...
def trace_event(name: str, **kwargs) -> None:
    if not log.isEnabledFor(logging.DEBUG):
        return
    parts = [f"{k}={v}" for k, v in kwargs.items()]
    log.debug("TRACE %s | %s", name, " ".join(parts))


def setup_logging(debug: bool = False, log_file: Optional[str] = None) -> Path:
    # L2-2 is a diagnostics-heavy build: force verbose logging.
    level = logging.DEBUG
    ...
    log.info(
        "logging initialized: level=%s (forced diagnostics), file=%s",
...
class ArmController:
    ...
    def _send_http(self, payload: Dict, timeout: float, accept_timeout: bool) -> bool:
        ...
        t0 = time.monotonic()
        trace_event("http_send", cmd=self.command_counter, timeout=timeout, accept_timeout=accept_timeout, payload=payload)
        try:
            response = self._session.get(url, timeout=timeout)
            response.raise_for_status()
            trace_event("http_ok", cmd=self.command_counter, code=response.status_code, elapsed_ms=int((time.monotonic() - t0) * 1000))
    ...
    def _query_position(self) -> Optional[Tuple[float, float, float]]:
        ...
        try:
            ...
            response.raise_for_status()
            data = response.json()
            trace_event("feedback_raw", data=data)
...
def run_game(play_white: bool, manual_mode: bool = False, max_moves: int = 0, resume: bool = True) -> int:
    ...
    trace_event("game_start", play_white=play_white, manual_mode=manual_mode, max_moves=max_moves, resume=resume)
    ...
    try:
        while True:
            ...
            digest = board_state_digest(board)
            log.info(
            ...
            trace_event("loop_begin", move_count=stats.move_count, next_action=next_action, white_idx=white_grave_idx, black_idx=black_grave_idx)
            trace_event("board_digest", digest=digest)
            if next_action == "wait_opponent":
                ...
                trace_event("opponent_done", next_action=next_action)
                save_checkpoint(board, white_grave_idx, black_grave_idx, next_action)
                continue
            ...
            trace_event("move_selected", move=move_str, speed=speed, think_s=f"{think_elapsed:.3f}")
            ...
            trace_event("checkpoint_save", next_action=next_action, move_count=stats.move_count)
            save_checkpoint(board, white_grave_idx, black_grave_idx, next_action)
