...
# ── Socket config ─────────────────────────────────────────────────────────────
SERVER_IP   = '10.194.26.222' #update this to server's IP address
SERVER_PORT = 9999
...
# Optional world->robot calibration scaffold from demo script.
# Keep disabled by default unless explicit calibration has been validated.
ROBOT_REALITY = {
    21: (232.5,  232.5),
    22: (232.5, -192.5),
    23: (-192.5, -192.5),
...
_world_pts = np.array([CORNER_WORLD[m] for m in [21, 22, 23, 24]], dtype=np.float32)
_robot_pts = np.array([ROBOT_REALITY[m] for m in [21, 22, 23, 24]], dtype=np.float32)
H_WORLD_TO_ROBOT, _ = cv2.findHomography(_world_pts, _robot_pts)
...
def apply_world_to_robot(wx, wy):
    """Map board-world coordinates to robot-world coordinates via calibration homography."""
    if H_WORLD_TO_ROBOT is None:
        return wx, wy
    pt = cv2.perspectiveTransform(
        np.array([[[float(wx), float(wy)]]], dtype=np.float32), H_WORLD_TO_ROBOT
    )
    return float(pt[0][0][0]), float(pt[0][0][1])


def get_piece_poses(ids, corners, H_matrix, use_robot_coords=False):
    """Return {piece_id: [(x, y), ...]} from current marker detections."""
    piece_data = {}
    if ids is None or H_matrix is None:
        return piece_data

    for i, marker_id in enumerate(ids.flatten()):
        if marker_id not in PIECE_IDS:
            continue

        c = corners[i][0]
        px = float(np.mean(c[:, 0]))
        py = float(np.mean(c[:, 1]))
        wx, wy = pixel_to_world(H_matrix, px, py)
        if use_robot_coords:
            wx, wy = apply_world_to_robot(wx, wy)

        piece_data.setdefault(int(marker_id), []).append((wx, wy))
...
class BoardPerception:
    """Import-safe perception interface used by hardware main loop."""
    ...
    def get_latest_state(self):
        ...
        frame, self.data_buffer = recv_frame(self.client_socket, self.data_buffer, self.payload_size)
        if frame is None:
            return None, None
        ...
        if ids is not None:
            ...
            if self.H_matrix is not None:
                board = build_board(ids, corners, self.H_matrix)
                poses = get_piece_poses(
                    ids,
                    corners,
                    self.H_matrix,
                    use_robot_coords=self.use_robot_coords,
                )

        # Consecutive-frame stability from demo script to suppress flicker.
        ...
        if self.stable_count >= self.stability_required:
            ...
            self.last_stable_poses = poses

        if self.last_stable_board is not None:
            return self.last_stable_board, self.last_stable_poses
